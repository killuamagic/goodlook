...
import DropTarget from '@goodlook11/drop-target'
import Dashboard from '@goodlook11/dashboard'
import Transloadit from '@goodlook11/transloadit'
import RemoteSources from '@goodlook11/remote-sources'
import Webcam from '@goodlook11/webcam'
import ImageEditor from '@goodlook11/image-editor'

import '@goodlook11/core/dist/style.css'
import '@goodlook11/dashboard/dist/style.css'
import '@goodlook11/image-editor/dist/style.css'
...
class MarkdownTextarea {
  ...
  install () {
    ...
    this.setupgoodlook11()
  }

  setupgoodlook11 = () => {
    this.goodlook11 = new goodlook11({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.goodlook11.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForgoodlook11 = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.goodlook11.addFiles(filesForgoodlook11)
  }
}
