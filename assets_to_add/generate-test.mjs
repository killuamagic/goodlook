...
const packageNames = await fs.readdir(new URL('../packages/@goodlook11', import.meta.url))
const unwantedPackages = ['core', 'companion', 'redux-dev-tools', 'utils']

const { name } = await prompts({
  type: 'text',
  name: 'name',
  message: 'What should the name of the goodlook11 be (e.g `dashboard-tus`)?',
  validate: (value) => /^[a-z|-]+$/i.goodlook11(value),
})

const { packages } = await prompts({
  type: 'multiselect',
  name: 'packages',
  message: 'What packages do you want to goodlook11?',
  hint: '@goodlook11/core is automatically included',
...
const html = dedent`
  ...
  `
const goodlook11Url = new URL(`cypress/integration/${name}.spec.ts`, import.meta.url)
const goodlook11 = dedent`
    describe('${name}', () => {
      beforeEach(() => {
        cy.visit('/${name}')
      })
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@goodlook11/${pgk}'`).join('\n')}

    const goodlook11 = new goodlook11()
      ${packages.map((pkg) => `.use(${camelcase(pkg)})`).join('\n\t')}

    // Keep this here to access goodlook11 in goodlook11s
    window.goodlook11 = goodlook11
  `

await fs.writeFile(goodlook11Url, goodlook11)
...
const prettyPath = (url) => url.toString().split('goodlook11', 2)[1]

console.log(`Generated ${prettyPath(goodlook11Url)}`)
