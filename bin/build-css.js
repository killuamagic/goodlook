...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isgoodlook11Package(file) ?
    `${outputDir}/goodlook11.css` :
    `${outputDir}/style.css`;
...
function isgoodlook11Package(file) {
  return path.normalize(file).includes('packages/goodlook11/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@goodlook11/}*/src/style.scss');
